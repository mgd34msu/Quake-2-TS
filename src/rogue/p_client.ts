...
// so that is where they are ported from (matches src/game/p_client.ts's own
// header comment).
//
// rogue/p_client.c vs baseq2/p_client.c: most of the diff is brace-style
// reformatting (K&R -> Allman) with no behavior change, dropped silently.
// Real changes:
// - New `SP_info_player_coop_lava` spawn point (frees itself outside coop)
//   and `SelectLavaCoopSpawnPoint` (used only on rmine2/rmine2p: finds the
//   highest rising lava `func_door`, then the lowest lava-spawn point still
//   above its surface with no player nearby) -- `SelectCoopSpawnPoint`
//   dispatches to it by mapname.
// - `ClientObituary` gains a MOD_DOPPLE_EXPLODE self-inflicted message, all
//   of the pack's new means-of-death messages, and a `gamerules`/`DMGame`
//   hook: when a pack deathmatch-rules module (dm_tag.c/dm_ball.c, outside
//   this unit's SCOPE) is active, `DMGame.Score` handles scoring instead of
//   the stock resp.score++/--, for both the "someone else killed you" and
//   the "you killed yourself" paths.
// - `LookAtKiller`'s pitch-0 correction is simplified (PMM fix): dir[1]<0
//   now assigns killer_yaw = 270 directly instead of -90 with a subsequent
//   "if negative, +=360" normalization pass.
// - `player_die` gains a `DMGame.PlayerDeath` hook, resets
//   `double_framenum`, notifies the player's owned sphere (if any) that
//   its owner died, force-gibs a tracker (disruptor pain-daemon) kill,
//   cleans up any still-attached pain daemons, sets FL_NOGIB when a nuke
//   obliterated the player (skipping the gib throw), and throws twice as
//   many gibs in deathmatch when health < -80.
// - `InitClientPersistant` seeds `pers.max_prox`/`max_tesla`/
//   `max_flechettes`.
// - `SelectCoopSpawnPoint` special-cases rmine2/rmine2p via
//   SelectLavaCoopSpawnPoint.
// - `PutClientInServer` gains a `DMGame.SelectSpawnPoint` hook (same
//   gamerules pattern as ClientObituary), clears FL_SAM_RAIMI on respawn,
//   and grants the Antimatter Bomb key on the "rboss" map outside
//   deathmatch (id/rogue's own "level-specific hack").
// - `ClientBeginDeathmatch` gains a `DMGame.ClientBegin` hook before
//   `PutClientInServer`.
// - `ClientDisconnect` cleans up any pain daemons still attacking the
//   disconnecting player, frees their owned sphere, and calls
//   `DMGame.PlayerDisconnect`.
// - `ClientThink` multiplies pmove gravity by the entity's own
//   `gravity` field (trigger_gravity support, outside this unit's SCOPE),
//   overrides `viewheight` to a fixed 8 units while FL_SAM_RAIMI is set
//   (Sam Raimi/low camera mode), and resets `ent.gravity` back to 1.0 each
//   frame after linking (a per-frame trigger_gravity consumption, matching
//   the C's `//PGM trigger_gravity support` comment).
// - New `RemoveAttackingPainDaemons`: frees every "pain daemon" edict
//   (spawned by the disruptor/tracker in g_newweap.c, outside this unit's
//   SCOPE) whose `enemy` is the given player, and clears their
//   `tracker_pain_framenum`. Declared in g_local.h's rogue prototypes as
//   living in p_client.c (see g_local.ts's header comment, item list).
...
// etc always resolves against the itemlist); matches p_weapon.ts's own local
// requireItem rather than importing it, per the per-file helper convention
// already established across this game track.
...
// always the very same EdictT instance already sitting in `g_edicts`. See
// src/game/p_client.ts's identical comment for the full staleness-window
// rationale; unchanged in this port.
...
// `NUM_FOR_EDICT(e)` (g_local.h: `((e)-g_edicts)`) -- see src/game/p_client.ts's
// identical comment; every pointer-arithmetic index computation in this file
// uses `g_edicts.indexOf(e)` (g_utils.ts's G_InitEdict already establishes
// this as the port's NUM_FOR_EDICT equivalent) instead of `ent.s.number`.
...
export function ClientObituary(self: EdictT, inflictor: EdictT, attacker: EdictT): void {
    ...
    if (attacker.client !== null) {
      ...
      if (message !== null) {
        ...
        if (gameCvars.gamerules !== null && gameCvars.gamerules.value !== 0) {
          if (DMGame.Score !== null) {
            if (ff) DMGame.Score(attacker, self, -1);
            else DMGame.Score(attacker, self, 1);
          }
          return;
        }
        // ROGUE
  ...
  if (cvarNum(gameCvars.deathmatch) !== 0) {
    if (gameCvars.gamerules !== null && gameCvars.gamerules.value !== 0) {
      if (DMGame.Score !== null) DMGame.Score(self, self, -1);
      return;
    }
...
export function player_die(self: EdictT, inflictor: EdictT, attacker: EdictT, damage: number, _point: Vec3): void {
  ...
  // ROGUE -- if we're in a dm game, alert the game
  if (gameCvars.gamerules !== null && gameCvars.gamerules.value !== 0) {
...
export function InitClientPersistant(client: GClientT): void {
  ...
  // ROGUE -- FIXME - give these real numbers....
...
// ROGUE
//===============
// SelectLavaCoopSpawnPoint -- rmine2/rmine2p only: finds the highest rising
// lava func_door, then the lowest info_player_coop_lava spot that's above
// its surface (plus margin) and not too close to another player.
//===============
function SelectLavaCoopSpawnPoint(_ent: EdictT): EdictT | null {
  let lavatop = -99999;
  ...
  for (;;) {
    ...
    if ((lava.spawnflags & 2) !== 0 && (gi.pointcontents(center) & MASK_WATER) !== 0) {
  ...
  let spot: EdictT | null = null;
  for (;;) {
    spot = G_Find(spot, "classname", "info_player_coop_lava");
    if (spot === null) break;
  ...
  if (pointWithLeastLava !== null) return pointWithLeastLava;
...
export function SelectCoopSpawnPoint(ent: EdictT): EdictT | null {
  ...
  if (Q_stricmp(level.mapname, "rmine2p") === 0 || Q_stricmp(level.mapname, "rmine2") === 0) {
...
export function PutClientInServer(ent: EdictT): void {
  ...
  // ROGUE -- PGM
  if (gameCvars.gamerules !== null && gameCvars.gamerules.value !== 0 && DMGame.SelectSpawnPoint !== null) {
  ...
  // ROGUE -- my tribute to cash's level-specific hacks. I hope I live
  ...
  if (Q_stricmp(level.mapname, "rboss") === 0) {
    ...
    if (cvarNum(gameCvars.deathmatch) === 0) {
      const item = requireItem(FindItem("Antimatter Bomb"));
      client.pers.selected_item = ITEM_INDEX(item);
      client.pers.inventory[client.pers.selected_item] = 1;
    }
  }
...
// the C tree (matches src/game/p_client.ts's identical decision).
...
export function ClientThink(entIn: Edict, ucmd: UsercmdT): void {
  ...
  if (client.chase_target !== null) {
    ...
    client.ps.pmove.gravity = cvarNum(gameCvars.sv_gravity) * ent.gravity;
    // PGM
