// g_main.c
//
// rogue/g_main.c vs baseq2/g_main.c: CheckNeedPass and its "needpass" cvar
// are dropped entirely (matches g_local.ts's dropped `needpass` gameCvars
// entry -- no rogue .c file references it), and CheckDMRules gains a
// gamerules/DMGame.CheckDMRules hook right after the deathmatch-off early
// return: when a pack deathmatch-rules module (g_newdm.ts's InitGameRules,
// outside this unit's SCOPE) has installed a CheckDMRules callback via the
// `gamerules` cvar, it gets first refusal before the stock timelimit/
// fraglimit checks run.
...
// rogue/g_main.c drops CheckNeedPass entirely -- no rogue .c file
// references the "needpass" cvar (see g_local.ts's gameCvars comment).
...
export function CheckDMRules(): void {
  ...
  // ROGUE
  if (gameCvars.gamerules !== null && gameCvars.gamerules.value !== 0 && DMGame.CheckDMRules !== null) {
    if (DMGame.CheckDMRules() !== 0) return;
  }
  // ROGUE
