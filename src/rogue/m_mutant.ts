...
Ported from rogue/m_mutant.c.

rogue/m_mutant.c vs baseq2/m_mutant.c: banner swap plus one addition -- a new
`monsterinfo.blocked` callback (mutant_blocked, m_mutant.c:97) wired up in
SP_monster_mutant, used by SV_NewChaseDir (src/rogue/m_move.ts) when no
cardinal direction works. mutant_blocked tries a vertical jump over/under
obstacles first (blocked_checkjump, from g_newai.c -- outside this unit's
SCOPE) via a new mutant_jump_up/mutant_jump_down move pair selected by
mutant_jump_updown, then falls back to the existing blocked_checkplat
elevator-riding helper (also g_newai.c).

Deviation: the C `mutant_blocked` (m_mutant.c:97-107) falls off the end of a
non-void function when both blocked_checkjump and blocked_checkplat return
false -- undefined behaviour in C, not a deliberate id/Rogue design. Ported
with an explicit `return false;` for that fallthrough case, the only
behaviour consistent with every other blocked_check* caller in this codebase
returning false on "couldn't get unstuck".
*/
/*
==============================================================================
...
import { blocked_checkjump, blocked_checkplat } from "./g_newai";
...
// vertical-jump-over-obstacle move pair, dispatched from mutant_blocked via
// mutant_jump_updown depending on whether the enemy is above or below us.
...
function mutant_jump_wait_land(self: EdictT): void {
  if (self.groundentity === null) self.monsterinfo.nextframe = self.s.frame;
  else self.monsterinfo.nextframe = self.s.frame + 1;
}

const mutant_frames_jump_up: MframeT[] = [
  mf(ai_move, -8, null),
  mf(ai_move, -8, mutant_jump_up),
  mf(ai_move, 0, mutant_jump_wait_land),
  mf(ai_move, 0, null),
  mf(ai_move, 0, null),
];
const mutant_move_jump_up = new MmoveT();
mutant_move_jump_up.firstframe = F.FRAME_jump01;
mutant_move_jump_up.lastframe = F.FRAME_jump05;
mutant_move_jump_up.frame = mutant_frames_jump_up;
mutant_move_jump_up.endfunc = mutant_run;

const mutant_frames_jump_down: MframeT[] = [
  mf(ai_move, 0, null),
  mf(ai_move, 0, mutant_jump_down),
  mf(ai_move, 0, mutant_jump_wait_land),
  mf(ai_move, 0, null),
  mf(ai_move, 0, null),
];
const mutant_move_jump_down = new MmoveT();
mutant_move_jump_down.firstframe = F.FRAME_jump01;
mutant_move_jump_down.lastframe = F.FRAME_jump05;
mutant_move_jump_down.frame = mutant_frames_jump_down;
mutant_move_jump_down.endfunc = mutant_run;

function mutant_jump_updown(self: EdictT): void {
  const enemy = self.enemy;
  if (!enemy) return;

  if (enemy.s.origin[2] > self.s.origin[2]) self.monsterinfo.currentmove = mutant_move_jump_up;
  else self.monsterinfo.currentmove = mutant_move_jump_down;
}
...
===
*/
function mutant_blocked(self: EdictT, dist: number): boolean {
  ...
  // C falls off the end of a non-void function here (undefined behaviour,
  // m_mutant.c:97-107); every other blocked_check* caller in this codebase
  // treats "couldn't get unstuck" as false, so this does too.
  return false;
}
//ROGUE
//================
...
registerSaveFunction("m_mutant:mutant_blocked", mutant_blocked);
registerSaveFunction("m_mutant:mutant_jump_updown", mutant_jump_updown);
...
registerSaveMmove("m_mutant:mutant_move_jump_up", mutant_move_jump_up);
registerSaveMmove("m_mutant:mutant_move_jump_down", mutant_move_jump_down);
