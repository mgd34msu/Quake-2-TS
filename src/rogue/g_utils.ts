...
//
// rogue/g_utils.c vs baseq2/g_utils.c: banner swap plus rogue additions --
// G_ProjectSource2 (adds an `up` basis vector alongside forward/right),
// findradius2 (a findradius variant restricted to SVF_DAMAGEABLE entities,
// used by the tesla's damage-radius scan), a train-chain cleanup pass in
// G_UseTargets' killtarget loop (removes a killtargeted train car from its
// master's teamchain before freeing it), a pitch-of-0 bugfix applied to both
// vectoyaw/vectoangles and their new vectoyaw2/vectoangles2 float-precision
// siblings (vectoyaw2/vectoangles2 skip the `(int)` truncation the original
// functions do), and a G_InitEdict addition that clears a stale nextthink
// left over from a bug elsewhere plus initializes the new gravityVector
// field (ROGUE_GRAVITY is unconditionally on in the shipped rogue binary,
// per g_local.ts's ROGUE_GRAVITY export).
...
import { type Edict, SolidT, SVF_DAMAGEABLE, SVF_MONSTER } from "./game";
import {
  BODY_QUEUE_SIZE,
...
// ROGUE
export function G_ProjectSource2(
  point: Vec3,
  distance: Vec3,
  forward: Vec3,
  right: Vec3,
...
export function findradius2(from: EdictT | null, org: Vec3, rad: number): EdictT | null {
  ...
  for (let i = start; i < globals.num_edicts; i++) {
    ...
    if ((candidate.svflags & SVF_DAMAGEABLE) === 0) continue;
    for (let j = 0; j < 3; j++) {
      eorg[j] = org[j] - (candidate.s.origin[j] + (candidate.mins[j] + candidate.maxs[j]) * 0.5);
    }
    if (VectorLength(eorg) > rad) continue;
...
export function G_UseTargets(ent: EdictT, activator: EdictT | null): void {
  ...
  if (ent.killtarget !== null) {
    ...
    while ((t = G_Find(t, "targetname", killtarget)) !== null) {
      // ROGUE -- if this entity is part of a train, cleanly remove it from
      // its master's teamchain before freeing it, so the train doesn't walk
      // a dangling pointer into a freed edict next time it thinks.
      if ((t.flags & FL_TEAMSLAVE) !== 0) {
...
// PMM - fixed to correct for pitch of 0 (rogue's g_utils.c comment) -- the
// base version's yaw==0 case fell through without setting `yaw` when
// vec[YAW] was also 0; rogue's fix makes the three-way branch explicit.
...
// ROGUE -- same fix as vectoyaw, but without the `(int)` truncation the C
// original applies (rogue's vectoyaw2 keeps the float precision of atan2).
export function vectoyaw2(vec: Vec3): number {
  let yaw: number;
  ...
  if (/*vec[YAW] == 0 &&*/ vec[PITCH] === 0) {
    ...
    yaw = (Math.atan2(vec[YAW], vec[PITCH]) * 180) / M_PI;
    if (yaw < 0) yaw += 360;
  }
...
// PMM - fixed to correct for pitch of 0: the base version's `else yaw = -90;`
// branch (vec[0] falsy, vec[1] <= 0) becomes `yaw = 270` in rogue.
...
// ROGUE -- same fix as vectoangles, but without the `(int)` truncation the C
// original applies to yaw/pitch.
...
export function G_InitEdict(e: EdictT): void {
  // ROGUE
  // FIXME -
  //   this fixes a bug somewhere that is settling "nextthink" for an entity
  //   that has already been released.  nextthink is being set to FRAMETIME
  //   after level.time, since freetime = nextthink - 0.1
  if (e.nextthink) {
    e.nextthink = 0;
  }
  // ROGUE
  ...
  // (see g_local.ts's ROGUE_GRAVITY export), so this always runs here.
