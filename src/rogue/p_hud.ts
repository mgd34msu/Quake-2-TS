...
// stub) at src/rogue/g_cmds.ts's `ValidateSelectedItem` export. Nothing here
// needs it.
//
// `Cmd_Score_f`/`Cmd_Help_f` are declared in g_local.h and called from
// g_cmds.c's ClientCommand, but defined here in p_hud.c; src/rogue/g_cmds.ts
// imports them from here directly.
//
// rogue/p_hud.c vs baseq2/p_hud.c: MoveClientToIntermission also clears the
// new ROGUE powerup timers (RDF_IRGOGGLES, ir_framenum, nuke_framenum,
// double_framenum); DeathmatchScoreboardMessage lets an active DMGame
// (gamerules cvar set) override the dogtag picture via DMGame.DogTag;
// G_SetStats' powerup timer chain gains a double_framenum tier (after quad)
// and an owned_sphere / ir_framenum tier (after breather).
...
export function DeathmatchScoreboardMessage(ent: EdictT, killer: EdictT | null): void {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // ROGUE -- allow new DM games to override the tag picture. C calls
    // DogTag(cl_ent, killer, &tag) unconditionally here, including when
    // killer is NULL; g_local.ts's DmGameRt.DogTag takes `EdictT | null` for
    // exactly this call site, so no killer-null guard is needed. C's `&tag`
    // is a conditionally-written out-param -- when DogTag doesn't write
    // through it, `tag` keeps whatever the tag1/tag2/NULL chain above set.
    // dm_tag.ts's Tag_DogTag documents "" as its "leave unchanged" sentinel
    // for exactly this reason, so only a non-empty return overwrites `tag`.
    if (gameCvars.gamerules !== null && gameCvars.gamerules.value !== 0 && DMGame.DogTag !== null) {
      const override = DMGame.DogTag(cl_ent, killer);
...
export function G_SetStats(ent: EdictT): void {
  ...
  if (client.quad_framenum > level.framenum) {
    ...
    if (client.owned_sphere.spawnflags === 1)
      client.ps.stats[STAT_TIMER_ICON] = gi.imageindex("p_defender");
    else if (client.owned_sphere.spawnflags === 2)
      client.ps.stats[STAT_TIMER_ICON] = gi.imageindex("p_hunter");
    else if (client.owned_sphere.spawnflags === 4)
      client.ps.stats[STAT_TIMER_ICON] = gi.imageindex("p_vengeance");
