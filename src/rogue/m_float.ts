...
Ported from rogue/m_float.c.

rogue/m_float.c vs baseq2/m_float.c (ported at src/game/m_float.ts): banner
swap plus a real gameplay delta --
  - floater_fire_blaster gains an early-out guard, `if (!self->enemy ||
    !self->enemy->inuse) return;` (m_float.c:51-52, "PGM"), run before the
    effect/frame check. This makes the base port's later inline
    `self.enemy === null` dereference-site guard unreachable, so it is
    replaced by a single `enemy` local captured right after the new guard.
  - A new circle-strafe attack variant: floater_frames_attack1a /
    floater_move_attack1a (same FRAME_attak101..FRAME_attak114 range as
    floater_move_attack1, m_float.c:245-266) plays the blaster loop while
    ai_charging with a fixed dist of 10 instead of 0.
  - floater_attack is rewritten to roll a skill-scaled chance (0% easy, 50%
    normal, 75% hard, 86.67% nightmare) between the straight attack1 move
    (AS_STRAIGHT) and the new circle-strafe attack1a move (AS_SLIDING,
    toggling monsterinfo.lefty on a coin flip), m_float.c:559-583.
  - A new floater_blocked callback (m_float.c:639-647, "PGM") delegates to
    g_newai.c's blocked_checkshot with a skill-scaled shot chance, wired to
    monsterinfo.blocked in SP_monster_floater (m_float.c:694).
*/
/*
==============================================================================
...
function floater_fire_blaster(self: EdictT): void {
  ...
  // PGM: bail out before firing if we've lost the enemy we're tracking.
...
// rows but FRAME_actvat01(0)..FRAME_actvat31(30) spans 31 (rogue/m_float.c:222-255,
// unchanged from baseq2/m_float.c:205-238). Preserved byte-for-byte; already
// dead code in the original per the comment above.
...
// PMM - circle strafe frames (rogue/m_float.c:245-266). Same frame range as
// floater_move_attack1 (14 rows, FRAME_attak101..FRAME_attak114).
const floater_frames_attack1a: MframeT[] = [
  mframe(ai_charge, 10), // Blaster attack
  mframe(ai_charge, 10),
  mframe(ai_charge, 10),
  mframe(ai_charge, 10, floater_fire_blaster), // BOOM (0, -25.8, 32.5)	-- LOOP Starts
  mframe(ai_charge, 10, floater_fire_blaster),
  mframe(ai_charge, 10, floater_fire_blaster),
  mframe(ai_charge, 10, floater_fire_blaster),
  mframe(ai_charge, 10, floater_fire_blaster),
  mframe(ai_charge, 10, floater_fire_blaster),
  ...
  mframe(ai_charge, 10),
  mframe(ai_charge, 10),
  mframe(ai_charge, 10), //							-- LOOP Ends
];
const floater_move_attack1a = mmove(FRAME_attak101, FRAME_attak114, floater_frames_attack1a, floater_run);
...
function floater_attack(self: EdictT): void {
  ...
  if (random() > chance) {
    ...
    // circle strafe
    if (random() <= 0.5) self.monsterinfo.lefty = 1 - self.monsterinfo.lefty;
...
//===========
//PGM
function floater_blocked(self: EdictT, _dist: number): boolean {
...
registerSaveFunction("m_float:floater_blocked", floater_blocked);
...
registerSaveMmove("m_float:floater_move_attack1a", floater_move_attack1a);
