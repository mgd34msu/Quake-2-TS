// g_cmds.c
//
// rogue/g_cmds.c vs baseq2/g_cmds.c: IT_NOT_GIVEABLE gates `give`/`give all`
// (Cmd_Give_f), a spawned give-item that fails to actually spawn (some
// rogue items refuse to spawn under certain conditions) short-circuits
// before Touch_Item, Cmd_Kill_f cleans up tracker pain daemons and an owned
// sphere before dying, Cmd_WeapPrev_f/Cmd_WeapNext_f swap scan direction and
// check `cl->newweapon` instead of `cl->pers.weapon` ("PMM - prevent
// scrolling through ALL weapons"), and ClientCommand gains "entcount"
// (Cmd_Ent_Count_f) and "disguise" (sets FL_DISGUISED) stringcmds.
...
import { player_die, RemoveAttackingPainDaemons } from "./p_client";
...
export function Cmd_Give_f(ent: EdictT): void {
  ...
  if (giveAll) {
    ...
    for (let i = 0; i < game.num_items; i++) {
      ...
      if (!it.pickup) continue;
      // ROGUE
      if (it.flags & IT_NOT_GIVEABLE) continue;
      // ROGUE
...
export function Cmd_Kill_f(ent: EdictT): void {
  ...
  if (client.tracker_pain_framenum) RemoveAttackingPainDaemons(ent);
...
=================
*/
export function Cmd_Ent_Count_f(ent: EdictT): void {
  let x = 0;

  for (let i = 0; i < globals.num_edicts; i++) {
    if (g_edicts[i].inuse) x++;
  }

  gi.dprintf(`${x} entites active\n`);
}
// ROGUE
