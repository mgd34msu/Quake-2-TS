// g_misc.c
//
// rogue/g_misc.c vs baseq2/g_misc.c: ThrowGib's spawned gibs get
// RF_IR_VISIBLE; func_explosive_explode cleans itself out of its train's
// teamchain if FL_TEAMSLAVE; func_explosive gains an INACTIVE spawnflag
// (func_explosive_activate, triggered only by the entity that targets it,
// arms the entity for both use and damage-triggered explosion); the barrel
// (misc_explobox) gets a real per-frame think (barrel_think/barrel_start,
// "so barrels will think and hence, blow up" -- calls the now-needed
// M_CatagorizePosition/M_WorldEffects from g_monster.ts) instead of a single
// M_droptofloor call; misc_deadsoldier_die's re-gib health threshold
// tightens from -80 to -30; and a new misc_nuke_core entity toggles
// SVF_NOCLIENT on use.
//
// M_WorldEffects is `extern`-declared at the top of rogue/g_misc.c (it is
// private/non-exported in this port's base g_monster.ts); the sibling unit
// porting g_monster.ts for rogue must export it to match -- see this unit's
// final report.
...
import { M_CatagorizePosition, M_droptofloor, M_WorldEffects } from "./g_monster";
...
export function ClipGibVelocity(ent: EdictT): void {
...
export function gib_touch(self: EdictT, _other: EdictT, plane: CplaneT | null, _surf: CsurfaceT | null): void {
...
export function gib_die(self: EdictT, _inflictor: EdictT, _attacker: EdictT, _damage: number, _point: Vec3): void {
  G_FreeEdict(self);
}
...
function func_explosive_explode(self: EdictT, inflictor: EdictT, attacker: EdictT, _damage: number, _point: Vec3): void {
  ...
  // ROGUE -- "PMM - if we're part of a train, clean ourselves out of it"
  if (self.flags & FL_TEAMSLAVE) {
    if (self.teammaster !== null) {
      ...
      if (master !== null && master.inuse) {
        let done = false;
        while (!done) {
          // C dereferences `master->teamchain` unconditionally at the top of
          // this loop on every iteration after the first; if self is never
          // found in the chain, `master` eventually goes NULL and the
          // original crashes. Preserved as a throw rather than a silent
          // early exit, per this port's established "unconditional C deref"
          // idiom (see requireEdict/requireOwner elsewhere in this unit).
          if (master === null) {
            throw new Error(
              "func_explosive_explode: ran off the end of the team chain without finding self (C dereferences master->teamchain unconditionally here)",
            );
          }
...
// ROGUE -- INACTIVE spawnflag: only the entity that targets this one may
// arm it (for both use and damage-triggered explosion). "PMM - looked like
// target and targetname were flipped here" -- the check below is the
// corrected direction (other/activator's `target` against self's
// `targetname`), not self's `target`.
function func_explosive_activate(self: EdictT, other: EdictT | null, activator: EdictT | null): void {
  let approved = false;
  ...
  // PMM - according to mappers, they don't need separate cases for
  // blowupable and triggerable
...
export function SP_func_explosive(self: EdictT): void {
  ...
  if (self.spawnflags & 1) {
    ...
    // ROGUE -- INACTIVE
    self.solid = SolidT.SOLID_BSP;
    if (self.targetname) self.use = func_explosive_activate;
    // ROGUE
  } else {
    self.solid = SolidT.SOLID_BSP;
...
// ROGUE -- "PGM - change so barrels will think and hence, blow up"
function barrel_think(self: EdictT): void {
...
function barrel_start(self: EdictT): void {
  M_droptofloor(self);
  self.think = barrel_think;
  self.nextthink = level.time + FRAMETIME;
}
...
function misc_deadsoldier_die(self: EdictT, _inflictor: EdictT, _attacker: EdictT, damage: number, _point: Vec3): void {
  // ROGUE -- threshold tightened from -80 to -30
...
registerSaveFunction("g_misc:func_explosive_activate", func_explosive_activate);
...
registerSaveFunction("g_misc:barrel_think", barrel_think);
registerSaveFunction("g_misc:barrel_start", barrel_start);
...
registerSaveFunction("g_misc:misc_nuke_core_use", misc_nuke_core_use);
