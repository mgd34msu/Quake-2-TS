...
//
// rogue/p_trail.c vs baseq2/p_trail.c: banner-only diff, no functional
// delta. Copied verbatim from src/game/p_trail.ts with imports adjusted to
// the rogue-local g_local.ts/g_ai.ts/g_utils.ts (g_ai.ts and g_utils.ts are
// owned by a sibling unit porting the remaining shared-basename files;
// imported here as if already present at their expected path per this
// unit's brief).
