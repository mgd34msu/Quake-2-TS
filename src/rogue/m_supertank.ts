...
Ported from rogue/m_supertank.c (GNU GPL v2 or later).
*/
/*
==============================================================================
...
*/
//
// rogue/m_supertank.c vs baseq2/m_supertank.c: banner swap plus four
// additions --
// 1. supertankRocket gains `if(!self->enemy || !self->enemy->inuse) return;`
//    as its first statement (rogue/m_supertank.c:485-486, "//PGM"), moving
//    the enemy-null guard ahead of the flash_number/AngleVectors/
//    G_ProjectSource work instead of the base port's late defensive check.
// 2. supertankMachineGun gains the same guard as its first statement
//    (rogue/m_supertank.c:514-515, "//PGM"), so unlike baseq2 (which fires
//    with a straight-ahead aim when there's no enemy) rogue skips the shot
//    entirely.
// 3. A new supertank_blocked function (rogue/m_supertank.c:656-667, wrapped
//    in "//PGM"/"//PGM" markers) wired to `self->monsterinfo.blocked` in
//    SP_monster_supertank (rogue/m_supertank.c:715). blocked_checkshot and
//    blocked_checkplat are defined in g_newai.c, owned by the RG-systems
//    unit (outside this unit's SCOPE); imported as if that module already
//    exists, per this batch's brief.
// 4. SP_monster_supertank sets `self->monsterinfo.aiflags |=
//    AI_IGNORE_SHOTS;` right after walkmonster_start(self)
//    (rogue/m_supertank.c:723-725, "//PMM"/"//pmm").
// Everything else is copied from src/game/m_supertank.ts with sibling
// imports repointed at the flat src/rogue/ layout.
...
function supertankRocket(self: EdictT): void {
  ...
  // ROGUE (rogue/m_supertank.c:485-486, "//PGM"): guard moved ahead of the
  // flash_number/AngleVectors/G_ProjectSource work, unlike baseq2's
...
function supertankMachineGun(self: EdictT): void {
  ...
  // ROGUE (rogue/m_supertank.c:514-515, "//PGM"): baseq2 fires with a
  // straight-ahead aim when there's no enemy; rogue skips the shot entirely.
  if (self.enemy === null || !self.enemy.inuse) return;
...
// ROGUE (rogue/m_supertank.c:656-667, "//PGM"/"//PGM")
function supertank_blocked(self: EdictT, dist: number): boolean {
...
export function SP_monster_supertank(self: EdictT): void {
  ...
  self.monsterinfo.blocked = supertank_blocked; // ROGUE (rogue/m_supertank.c:715, "//PGM")
  ...
  // ROGUE (rogue/m_supertank.c:723-725, "//PMM"/"//pmm")
...
registerSaveFunction("m_supertank:supertank_blocked", supertank_blocked);
