...
//
// rogue/g_save.c vs baseq2/g_save.c: this is where InitGame() lives, and it
// is the one file every rogue EdictT/GClientT/MonsterInfoT/LevelLocalsT/
// ClientPersistantT field addition (g_local.ts) ultimately has to be
// threaded through, since this port's save/load path is a hand-written
// property-by-property JSON walk rather than a byte-offset table walk (see
// the design note above). Concretely:
//   - InitGame's cvar registration list drops `needpass` (rogue never
//     references it -- see g_local.ts's gameCvars comment) and adds
//     sv_stopspeed (promoted from a g_phys.c #define to a real cvar),
//     g_showlogic, gamerules, huntercam, strong_mines, randomrespawn; and
//     calls InitGameRules() (g_newdm.ts, RG-systems' SCOPE) once gamerules
//     is registered, matching rogue's `if(gamerules) InitGameRules();`.
//   - C's `fields[]` table (used by ED_ParseField for spawn-time text
//     parsing) gains a block of new F_EDICT/F_MMOVE(FFL_NOSPAWN) entries
//     (bad_area, hint_chain, monster_hint_chain, target_hint_chain,
//     goal_hint, badMedic1, badMedic2, last_player_enemy, commander,
//     blocked/duck/unduck/sidestep) and `levelfields[]`/`clientfields[]`
//     gain disguise_violator/owned_sphere. None of these need a new FIELDS
//     entry here: FIELDS (this port's redesign of that table) already only
//     carries the spawn-parseable subset (F_LSTRING/F_INT/F_FLOAT/F_VECTOR/
//     F_ANGLEHACK) per this file's own comment below -- every F_EDICT/
//     F_MMOVE/F_FUNCTION/F_ITEM entry, in both baseq2 and rogue, is excluded
//     from ED_ParseField's reachable set the same way and is handled by the
//     hand-written serializeEdict/deserializeEdict (etc.) walk instead. All
//     of rogue's new EdictT/MonsterInfoT/ClientPersistantT/LevelLocalsT
//     fields are added to those JSON shapes below so the actual save/load
//     data is complete.
//   - MonsterInfoT.dodge widened to take the triggering trace (g_local.ts);
//     wrapDodge below matches the new 4-arg shape.
...
import { InitGameRules } from "./g_newdm";
...
// ROGUE -- dodge widened to take the trace that triggered it (g_local.ts)
function wrapDodge(fn: Function): (self: EdictT, other: EdictT, eta: number, tr: GTraceT) => void {
  return (self: EdictT, other: EdictT, eta: number, tr: GTraceT) => {
    Reflect.apply(fn, null, [self, other, eta, tr]);
  };
}
function wrapCheckattack(fn: Function): (self: EdictT) => boolean {
  return (self: EdictT) => Boolean(Reflect.apply(fn, null, [self]));
}
// ROGUE -- new MonsterInfoT callback shapes
function wrapBlockedDist(fn: Function): (self: EdictT, dist: number) => boolean {
  return (self: EdictT, dist: number) => Boolean(Reflect.apply(fn, null, [self, dist]));
}
function wrapVoidSelfEta(fn: Function): (self: EdictT, eta: number) => void {
  return (self: EdictT, eta: number) => {
    Reflect.apply(fn, null, [self, eta]);
  };
}
// ROGUE
...
function populateSaveRegistry(): void {
  const entries: Array<[string, Function]> = [
    ...
    ["trigger_push_use", trigger_push_use], // ROGUE
    ["hurt_touch", hurt_touch],
    ["trigger_gravity_touch", trigger_gravity_touch],
    ["trigger_gravity_use", trigger_gravity_use], // ROGUE
...
interface MonsterInfoJSON {
  ...
  duck: string | null;
  unduck: string | null;
  sidestep: string | null;
  base_height: number;
  next_duck_time: number;
  duck_wait_time: number;
  last_player_enemy: number;
  blindfire: boolean;
  blind_fire_delay: number;
  blind_fire_target: number[];
  monster_slots: number;
  monster_used: number;
  commander: number;
  quad_framenum: number;
  invincible_framenum: number;
  double_framenum: number;
  // ROGUE
}
...
function serializeMonsterinfo(mi: MonsterInfoT): MonsterInfoJSON {
  return {
    ...
    // ROGUE
    blocked: nameOfFunction(mi.blocked),
    last_hint_time: mi.last_hint_time,
    goal_hint: edictIndex(mi.goal_hint),
    medicTries: mi.medicTries,
    badMedic1: edictIndex(mi.badMedic1),
    badMedic2: edictIndex(mi.badMedic2),
    healer: edictIndex(mi.healer),
    duck: nameOfFunction(mi.duck),
    unduck: nameOfFunction(mi.unduck),
    sidestep: nameOfFunction(mi.sidestep),
    base_height: mi.base_height,
    next_duck_time: mi.next_duck_time,
    duck_wait_time: mi.duck_wait_time,
    last_player_enemy: edictIndex(mi.last_player_enemy),
    blindfire: mi.blindfire,
    blind_fire_delay: mi.blind_fire_delay,
    blind_fire_target: Array.from(mi.blind_fire_target),
    monster_slots: mi.monster_slots,
    monster_used: mi.monster_used,
    commander: edictIndex(mi.commander),
    quad_framenum: mi.quad_framenum,
    invincible_framenum: mi.invincible_framenum,
    double_framenum: mi.double_framenum,
    // ROGUE
  };
}
...
function deserializeMonsterinfo(json: MonsterInfoJSON): MonsterInfoT {
  ...
  mi.blocked = blocked === null ? null : wrapBlockedDist(blocked);
  mi.last_hint_time = json.last_hint_time;
  mi.goal_hint = edictFromIndex(json.goal_hint);
  mi.medicTries = json.medicTries;
  mi.badMedic1 = edictFromIndex(json.badMedic1);
  mi.badMedic2 = edictFromIndex(json.badMedic2);
  mi.healer = edictFromIndex(json.healer);
  const duck = json.duck === null ? null : lookupSaveFunctionRaw(json.duck);
  mi.duck = duck === null ? null : wrapVoidSelfEta(duck);
  const unduck = json.unduck === null ? null : lookupSaveFunctionRaw(json.unduck);
  mi.unduck = unduck === null ? null : wrapVoidSelf(unduck);
  const sidestep = json.sidestep === null ? null : lookupSaveFunctionRaw(json.sidestep);
  mi.sidestep = sidestep === null ? null : wrapVoidSelf(sidestep);
  mi.base_height = json.base_height;
  mi.next_duck_time = json.next_duck_time;
  mi.duck_wait_time = json.duck_wait_time;
  mi.last_player_enemy = edictFromIndex(json.last_player_enemy);
  mi.blindfire = json.blindfire;
  mi.blind_fire_delay = json.blind_fire_delay;
  setVec3(mi.blind_fire_target, json.blind_fire_target);
  mi.monster_slots = json.monster_slots;
  mi.monster_used = json.monster_used;
  mi.commander = edictFromIndex(json.commander);
  mi.quad_framenum = json.quad_framenum;
  mi.invincible_framenum = json.invincible_framenum;
  mi.double_framenum = json.double_framenum;
  // ROGUE
  return mi;
}
...
export interface EdictJSON {
  ...
  bad_area: number;
  hint_chain: number;
  monster_hint_chain: number;
  target_hint_chain: number;
...
export function serializeEdict(ent: EdictT): EdictJSON {
  return {
    ...
    offset: Array.from(ent.offset),
    gravityVector: Array.from(ent.gravityVector),
    bad_area: edictIndex(ent.bad_area),
    hint_chain: edictIndex(ent.hint_chain),
    monster_hint_chain: edictIndex(ent.monster_hint_chain),
    target_hint_chain: edictIndex(ent.target_hint_chain),
    hint_chain_id: ent.hint_chain_id,
    lastMoveTime: ent.lastMoveTime,
    // ROGUE
  };
}
...
export function deserializeEdict(ent: EdictT, json: EdictJSON): void {
  ...
  setVec3(ent.offset, json.offset);
  setVec3(ent.gravityVector, json.gravityVector);
  ent.bad_area = edictFromIndex(json.bad_area);
  ent.hint_chain = edictFromIndex(json.hint_chain);
  ent.monster_hint_chain = edictFromIndex(json.monster_hint_chain);
  ent.target_hint_chain = edictFromIndex(json.target_hint_chain);
  ent.hint_chain_id = json.hint_chain_id;
  ent.lastMoveTime = json.lastMoveTime;
  // ROGUE
}
...
export interface LevelJSON {
  ...
  power_cubes: number;
  // ROGUE
  disguise_violator: number;
  disguise_violation_framenum: number;
  // ROGUE
}
...
export function serializeLevel(): LevelJSON {
  return {
    ...
    // ROGUE
    disguise_violator: edictIndex(level.disguise_violator),
    disguise_violation_framenum: level.disguise_violation_framenum,
    // ROGUE
  };
}
...
export function deserializeLevel(json: LevelJSON): void {
  ...
  // ROGUE
  level.disguise_violator = edictFromIndex(json.disguise_violator);
  level.disguise_violation_framenum = json.disguise_violation_framenum;
  // ROGUE
...
export function InitGame(): void {
  ...
  // ROGUE -- promoted from a g_phys.c #define to a real cvar
  gameCvars.sv_stopspeed = gi.cvar("sv_stopspeed", "100", 0);
  ...
  gameCvars.gamerules = gi.cvar("gamerules", "0", CVAR_LATCH); // ROGUE
  ...
  // rogue/g_save.c drops `needpass = gi.cvar("needpass", "0", CVAR_SERVERINFO);`
  // entirely -- see g_local.ts's gameCvars comment.
  ...
  // ROGUE -- `if(gamerules) InitGameRules();`: gamerules is always non-null
  // here (just assigned above via gi.cvar()), same as the C pointer check.
  if (gameCvars.gamerules !== null) {
    InitGameRules();
  }
  // ROGUE
}
