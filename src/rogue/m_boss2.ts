...
Ported from rogue/m_boss2.c (GNU GPL v2 or later).

rogue/m_boss2.c vs baseq2/m_boss2.c: adds a predictive-rocket-lead attack
(Boss2PredictiveRocket, taken 90% of the time against a client enemy) with
a widened rocket spread on the non-predictive fallback, triples the
machinegun horizontal spread and flips its lead-velocity sign, speeds up
several move tables (walk/run/attack frame distances raised from 8/1 to
10/2, the attack_rocket ai_move dip from -20 to -5), lets the boss shoot at
info_notnull "enemies" (zero-solid fake targets) even without a clear trace
in Boss2_CheckAttack, and sets self.yaw_speed = 50 in SP_monster_boss2
(unset -- and therefore 0, per the C zero-initialized edict -- in the base
game).
*/
/*
==============================================================================
...
function Boss2PredictiveRocket(self: EdictT): void {
  ...
  let time = dist / BOSS2_ROCKET_SPEED;
...
function Boss2Rocket(self: EdictT): void {
  ...
  // ROGUE -- predictive lead shot against a real client enemy, most of the time
  if (self.enemy.client !== null && random() < 0.9) {
...
function boss2_firebullet_right(self: EdictT): void {
  ...
  // ROGUE: lead sign flipped from -0.2 to 0.2 versus base
  VectorMA(self.enemy.s.origin, 0.2, self.enemy.velocity, target);
  target[2] += self.enemy.viewheight;
  VectorSubtract(target, start, forward);
  VectorNormalize(forward);

  // ROGUE: horizontal spread tripled versus base
...
// (rogue/m_boss2.c:227 carries the identical bug).
const boss2_move_fidget = mkmove(FRAME.FRAME_stand1, FRAME.FRAME_stand30, boss2_frames_fidget, null, true);

// ROGUE: walk/run frame distance raised from 8 to 10 ("PMM - normally, this is all 8's .. I upped it")
...
// ROGUE: attack frame distance raised from 1 to 2 ("used to be all 1's .. what a slow guy")
const boss2_frames_attack_pre_mg: MframeT[] = [
  ...Array.from({ length: 8 }, () => mkframe(ai_charge, 2)),
...
function Boss2_CheckAttack(self: EdictT): boolean {
  ...
  if (self.enemy.health > 0) {
    ...
    if (traceEdict(tr.ent) !== self.enemy) {
      // ROGUE: shoot at info_notnulls (zero-solid fake targets) even
      // without a clear trace to them.
  ...
  // ROGUE: go ahead and shoot every time if it's an info_notnull
  if (random() < chance || self.enemy.solid === SolidT.SOLID_NOT) {
