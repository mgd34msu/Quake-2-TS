...
export function Cmd_Use_f(ent: EdictT): void {
  ...
  let index = ITEM_INDEX(it);
  if (!client.pers.inventory[index]) {
    // RAFAEL
    if (it.pickup_name === "HyperBlaster") {
      it = FindItem("Ionripper");
      index = it === null ? -1 : ITEM_INDEX(it);
      if (it === null || !client.pers.inventory[index]) {
  ...
  // C calls it->use(ent, it) unconditionally here even after the RAFAEL
  // fallback swaps `it` to Ionripper/Phalanx -- both are weapon items whose
  // .use is always set by the itemlist table, so this can't actually be
  // null in practice. TS can't express an unchecked deref through a
  // reassigned nullable field, so fail loudly instead of silently
  // miscompiling (see g_trigger.ts's requireEdict for the same idiom).
  if (it.use === null) gi.error(`Cmd_Use_f: ${s} has no use function (C dereferences it unconditionally here)`);
  it.use(ent, it);
}
...
export function Cmd_Drop_f(ent: EdictT): void {
  ...
  // See Cmd_Use_f's matching comment: C calls it->drop(ent, it) unconditionally
  // here even after the RAFAEL fallback swaps `it`; fail loudly rather than
  // silently miscompile if a fallback item somehow lacks a drop function.
  if (it.drop === null) gi.error(`Cmd_Drop_f: ${s} has no drop function (C dereferences it unconditionally here)`);
  it.drop(ent, it);
}
