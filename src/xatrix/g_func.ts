...
import {
  ...
  EF_ANIM_ALL,
  EF_ANIM_ALLFAST,
  // xatrix/g_func.c: rotating_light's EF_SPINNINGLIGHTS render effect.
  EF_SPINNINGLIGHTS,
  EntityEventT,
  MulticastT,
  Q_stricmp,
...
function Think_AccelMove(ent: EdictT): void {
  ...
  if (ent.moveinfo.current_speed === 0) plat_CalcAcceleratedMove(ent.moveinfo); // starting or blocked
...
function Use_Plat(ent: EdictT, _other: EdictT | null, _activator: EdictT | null): void {
...
export function SP_object_repair(ent: EdictT): void {
  ...
  // Preserved bug-for-bug per PORTING.md (xatrix/g_func.c: SP_object_repair).
...
registerSaveFunction("g_func:rotating_light_alarm", rotating_light_alarm);
registerSaveFunction("g_func:rotating_light_killed", rotating_light_killed);
registerSaveFunction("g_func:rotating_light_use", rotating_light_use);
registerSaveFunction("g_func:object_repair_fx", object_repair_fx);
registerSaveFunction("g_func:object_repair_dead", object_repair_dead);
registerSaveFunction("g_func:object_repair_sparks", object_repair_sparks);
