...
// xatrix/g_monster.c: `// RAFAEL` block
...
// route through this shared beam entity.
...
export function M_CheckGround(ent: EdictT): void {
  ...
  const point = vec3(ent.s.origin[0], ent.s.origin[1], ent.s.origin[2] - 0.25);
...
export function M_droptofloor(ent: EdictT): void {
  ent.s.origin[2] += 1;
  const end = vec3(ent.s.origin[0], ent.s.origin[1], ent.s.origin[2] - 256);
...
export function monster_triggered_spawn(self: EdictT): void {
  ...
  // xatrix/g_monster.c: `// RAFAEL` -- fixbot ignores its triggered enemy
  // when spawned in one of its scripted-order modes (owned by the sibling
  // XA-monsters unit's m_fixbot.ts).
