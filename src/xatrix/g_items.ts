...
export function Pickup_Powerup(ent: EdictT, other: EdictT): boolean {
  ...
  if (cvarNum(gameCvars.deathmatch) !== 0) {
    ...
    if ((dmflags & DF_INSTANT_ITEMS) !== 0 || isDroppedQuad) {
      ...
      // xatrix/g_items.c: `// RAFAEL` -- same instant-use/dropped-timeout
      // treatment as quad damage above, for the quadfire powerup.
      const isDroppedQuadFire = item.use === Use_QuadFire && (ent.spawnflags & DROPPED_PLAYER_ITEM) !== 0;
      if ((dmflags & DF_INSTANT_ITEMS) !== 0 || isDroppedQuadFire) {
        if (isDroppedQuadFire) {
          quad_fire_drop_timeout_hack = ((ent.nextthink - level.time) / FRAMETIME) | 0;
        }
        if (item.use !== null) item.use(other, item);
      }
    }
...
export function Add_Ammo(ent: EdictT, item: GItemT, count: number): boolean {
  ...
  // xatrix/g_items.c: `// RAFAEL` (two separate `else if` blocks in the C)
...
export function droptofloor(ent: EdictT): void {
  ...
  if (tr.startsolid) {
    // xatrix/g_items.c: `// RAFAEL` -- a foodcube (spawned in mid-air by
    // Trap_Think, see g_weapon.ts) that starts solid just settles at its
    // spawn origin instead of being freed.
...
const ITEMLIST: GItemT[] = fixedLength("ITEMLIST", 49, [
  ...
  */
  ...
  mkItem({
    ...
    pickup: Pickup_Weapon,
    // xatrix/g_items.c: `// RAFAEL` -- redirects to Ionripper if already owned.
    use: Use_Weapon2,
    drop: Drop_Weapon,
  ...
  mkItem({
    ...
    pickup: Pickup_Weapon,
    // xatrix/g_items.c: `// RAFAEL` -- redirects to Phalanx if already owned.
    use: Use_Weapon2,
    drop: Drop_Weapon,
...
// xatrix/g_items.c: `// RAFAEL` -- spawned directly by g_weapon.ts's
// Trap_Think (a G_Spawn() + SP_item_foodcube() call, not through the
// spawns[] registry) when the trap weapon finishes digesting a kill.
