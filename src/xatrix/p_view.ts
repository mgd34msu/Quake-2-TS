...
export function SV_CalcBlend(ent: EdictT): void {
  ...
  if (client.quad_framenum > level.framenum) {
    ...
    // RAFAEL
    const remaining = client.quadfire_framenum - level.framenum;
    if (remaining === 30) gi.sound(ent, CHAN_ITEM, gi.soundindex("items/quadfire2.wav"), 1, ATTN_NORM, 0);
    if (remaining > 30 || (remaining & 4) !== 0) SV_AddBlend(1, 0.2, 0.5, 0.08, client.ps.blend);
...
export function G_SetClientSound(ent: EdictT): void {
  ...
  // RAFAEL
  else if (weap === "weapon_phalanx") ent.s.sound = gi.soundindex("weapons/phaloop.wav");
