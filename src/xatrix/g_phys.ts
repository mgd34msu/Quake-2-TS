...
export function SV_Physics_Toss(ent: EdictT): void {
  ...
  // add gravity
  // xatrix/g_phys.c: `// RAFAEL -- move type for rippergun projectile`
  if (
  ...
  if (trace.fraction < 1) {
    let backoff: number;
    // xatrix/g_phys.c: `// RAFAEL` -- the ionripper's wallbounce projectile
    ...
    // stop if on ground
    // xatrix/g_phys.c: `&& ent->movetype != MOVETYPE_WALLBOUNCE` -- the
    // wallbounce projectile never settles on the ground like a real bounce.
...
export function G_RunEntity(ent: EdictT): void {
  ...
  switch (ent.movetype) {
    ...
    // xatrix/g_phys.c: `// RAFAEL` -- MOVETYPE_WALLBOUNCE routes through
    // the same toss/bounce physics as MOVETYPE_BOUNCE.
