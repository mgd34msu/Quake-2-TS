...
import {
  ...
  MOD_BLASTER,
  MOD_BOMB,
  // xatrix/p_client.c: `// RAFAEL` / `// RAFAEL 14-APR-98` blocks add
  // MOD_GEKK, MOD_BRAINTENTACLE, MOD_TRAP, MOD_RIPPER, MOD_PHALANX to the
  // three ClientObituary switches (environmental death, self-kill, and
  // attacker-kill messages).
  MOD_BRAINTENTACLE,
  MOD_CHAINGUN,
  MOD_CRUSH,
  MOD_EXIT,
...
export function TossClientWeapon(self: EdictT): void {
  ...
  // RAFAEL
  let quadfire: boolean;
  if (((cvarNum(gameCvars.dmflags) | 0) & DF_QUADFIRE_DROP) === 0) {
    quadfire = false;
  } else {
  ...
  if (quadfire) {
    self.client.v_angle[YAW] += spread;
    const drop = Drop_Item(self, requireItem(FindItemByClassname("item_quadfire")));
    self.client.v_angle[YAW] -= spread;
    drop.spawnflags |= DROPPED_PLAYER_ITEM;
