...
export enum AmmoT {
  ...
  AMMO_CELLS,
  AMMO_SLUGS,
  // xatrix/g_local.h: `// RAFAEL` -- AMMO_MAGSLUG (ionripper's magnetic
  // slugs) and AMMO_TRAP (thrown trap weapon) appended to ammo_t.
  AMMO_MAGSLUG,
  AMMO_TRAP,
}
...
export enum MovetypeT {
  ...
  MOVETYPE_BOUNCE,
  // xatrix/g_local.h: `// RAFAEL` -- the trap weapon's thrown grenade
  // bounces off walls instead of settling (see g_phys.c's Xatrix delta).
  MOVETYPE_WALLBOUNCE,
}
...
// end xatrix/g_local.h `// END 14-APR-98`
...
export class ClientPersistantT {
  ...
  max_cells = 0;
  max_slugs = 0;
  // xatrix/g_local.h: `// RAFAEL` -- ammo caps for the ionripper's magnetic
  // slugs and the thrown trap weapon.
  max_magslug = 0;
  max_trap = 0;
...
export class GClientT {
  ...
  // rate-of-fire powerup timer), trap_blew_up/trap_time (mirrors
  // grenade_blew_up/grenade_time for the thrown trap weapon).
  quadfire_framenum = 0;
  trap_blew_up = false;
  trap_time = 0;
  silencer_shots = 0;
...
export class EdictT implements Edict {
  ...
  // xatrix/g_local.h: `// RAFAEL` -- `int orders;` appended after
  // monsterinfo on edict_s (used by m_fixbot.c/m_gekk.c's order-following
...
export const gameCvars: {
  ...
  password: CvarT | null;
  // xatrix/g_local.h drops `extern cvar_t *needpass;` -- g_main.c's Xatrix
  // delta removes CheckNeedPass() and the needpass cvar entirely.
