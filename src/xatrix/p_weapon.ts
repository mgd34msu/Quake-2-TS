...
export function Pickup_Weapon(ent: EdictT, other: EdictT): boolean {
  ...
  if ((ent.spawnflags & DROPPED_ITEM) === 0) {
    ...
    if (item.ammo !== null && item.ammo.length > 0) {
      ...
      // "ammo_trap") and Pickup_Weapon is never invoked for the ammo_trap
      // item to begin with (its pickup fn is Pickup_Ammo) -- the added
      // condition is always nonzero, so this is dead code. Preserved
      // bug-for-bug rather than "fixed" into a functional trap exception.
...
// xatrix/p_weapon.c: `// RAFAEL 14-APR-98` -- HyperBlaster/Railgun's Use_
...
export function Weapon_Generic(
  ...
  // xatrix/p_weapon.c drops the "VWep animations screw up corpses" early
  // return that base Quake 2 has here.
