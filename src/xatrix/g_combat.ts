...
export function T_Damage(
  ...
  // xatrix/g_combat.c: `&& mod != MOD_TRAP` -- the trap weapon ignores
  // invincibility (it grapples and instakills rather than dealing damage).
  if (
    client !== null &&
  ...
  if (take) {
    if (targ.svflags & SVF_MONSTER || client !== null) {
      // xatrix/g_combat.c: `// RAFAEL` -- monster_gekk bleeds green.
      if (targ.classname === "monster_gekk") {
        SpawnDamage(TempEventT.TE_GREENBLOOD, point, normal, take);
      } else {
