// m_soldierh.h -- frame index constants
//
// Pack-only header (no baseq2 equivalent): xatrix/m_soldierh.h defines the
// exact same FRAME_* names/values and MODEL_SCALE as baseq2/m_soldier.h --
// diffing the two C headers byte-for-byte confirms the only difference is
// copyright-header noise. In C this is a harmless duplicate macro
// definition (xatrix/m_soldier.c #includes both m_soldier.h and
// m_soldierh.h; identical redefinitions are legal). Kept as its own module
// here (rather than re-exporting m_soldier_frames.ts) so the soldierh_*
// move tables reference the same macro *names* the C soldierh_frames_*
// tables do, via a separate `FRAMEH` import namespace in m_soldier.ts.
