...
import {
  ...
  EF_FLIES,
  EF_GIB,
  // RAFAEL: acid-blooded gib effect used by ThrowGibACID/ThrowHeadACID.
  EF_GREENGIB,
  EF_ROCKET,
  EF_TELEPORTER,
  EntityEventT,
  MASK_MONSTERSOLID,
  // RAFAEL: acid gibs get a shootable clipmask instead of SOLID_NOT.
  MASK_SHOT,
  MulticastT,
  MZ2_CHICK_ROCKET_1,
  PMF_TIME_TELEPORT,
...
import {
  ...
  MOD_CRUSH,
  MOD_EXPLOSIVE,
  // RAFAEL: misc_nuke's use_nuke instakills every non-monster... see below.
  MOD_TRAP,
  MovetypeT,
  svc_temp_entity,
} from "./g_local";
import { func_train_find, train_use } from "./g_func";
import { M_droptofloor, monster_fire_rocket } from "./g_monster";
...
// RAFAEL
// gib_touchacid (acid-blood pain touch for organic ThrowGibACID/
// ThrowHeadACID gibs) is `#if 0`'d out in xatrix/g_misc.c ("removed acid
// damage") and never compiles in the original -- dropped here as dead code,
// matching every other #if 0/#ifdef-disabled block in this port.

// RAFAEL
export function ThrowGibACID(self: EdictT, gibname: string, damage: number, gibType: number): void {
  const gib = G_Spawn();
  ...
  // C: `// gi.setmodel (gib, gibname);` is commented out -- the gib gets a
  // bare modelindex instead of the mins/maxs/solid setup gi.setmodel would
  // normally derive from the model.
  ...
  if (gibType === GIB_ORGANIC) {
    gib.movetype = MovetypeT.MOVETYPE_TOSS;
    // RAFAEL 24-APR-98: removed acid damage -- `gib->touch = gib_touchacid`
    // is commented out in the C source (see gib_touchacid note above).
    vscale = 3.0;
  } else {
    gib.movetype = MovetypeT.MOVETYPE_BOUNCE;
    vscale = 1.0;
...
// RAFAEL
export function ThrowHeadACID(self: EdictT, gibname: string, damage: number, gibType: number): void {
  self.s.skinnum = 0;
  self.s.frame = 0;
  VectorClear(self.mins);
  VectorClear(self.maxs);
  ...
  self.s.effects &= ~EF_FLIES;
  // C: `self->s.effects |= RF_FULLBRIGHT;` -- bug-for-bug preserved: this
  // ORs the renderfx bit RF_FULLBRIGHT into s.effects instead of
  // s.renderfx (unlike the otherwise-identical line in ThrowGibACID above,
  // which gets the field right).
  ...
  if (gibType === GIB_ORGANIC) {
    self.movetype = MovetypeT.MOVETYPE_TOSS;
    // RAFAEL 24-APR-98: removed acid damage -- `self->touch =
    // gib_touchacid` is commented out in the C source.
    vscale = 0.5;
  } else {
    self.movetype = MovetypeT.MOVETYPE_BOUNCE;
    vscale = 1.0;
...
function misc_viper_missile_use(self: EdictT, _other: EdictT | null, _activator: EdictT | null): void {
  ...
  // this file (e.g. teleporter_touch).
  if (self.enemy === null) {
...
registerSaveFunction("g_misc:misc_viper_missile_use", misc_viper_missile_use);
registerSaveFunction("g_misc:amb4_think", amb4_think);
registerSaveFunction("g_misc:use_nuke", use_nuke);
